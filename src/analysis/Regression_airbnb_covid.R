...
model_1 <- lm(log(avg_price) ~ room_type + minimum_nights + number_of_reviews  + reviews_per_month +
                availability_365 + number_of_reviews_ltm +
                weekly_count, merged_data)
summary(model_1)

model_2 <- lm(log(avg_price) ~ room_type + minimum_nights + weekly_count, merged_data)
summary(model_2)
